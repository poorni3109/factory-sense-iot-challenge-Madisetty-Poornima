...
The DATABASE_URL is configurable via environment variable for deployment
(e.g., Render uses an ephemeral filesystem, so the DB path matters).
"""
...
DATABASE_URL = os.getenv("DATABASE_URL", "sqlite:///./factory_sense.db")
