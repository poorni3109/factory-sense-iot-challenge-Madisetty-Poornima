...
Usage:
  python simulator.py                          # targets http://127.0.0.1:8000
  python simulator.py https://your-app.onrender.com  # targets deployed URL
...
# Accept API URL as CLI argument, or default to local
...
REQUEST_TIMEOUT = 30  # HTTP timeout (generous for cold-start on Render free tier)
...
def post_reading(device_id: str, temp: float, vibe: float):
    ...
    try:
        r = requests.post(f"{API_URL}/telemetry", json=payload, timeout=REQUEST_TIMEOUT)
        r.raise_for_status()
        data = r.json()
...
def main():
    ...
    # Connectivity check (with generous timeout for Render cold start)
    log.info("🔌 Checking server connectivity (may take ~30s on free-tier cold start)...")
    try:
        r = requests.get(f"{API_URL}/health", timeout=60)
        r.raise_for_status()
        log.info("✅ Server is reachable!")
    except Exception as e:
        log.error("❌ Cannot reach %s — %s", API_URL, e)
        log.error("   For local: uvicorn main:app --reload --host 0.0.0.0")
        log.error("   For deployed: python simulator.py https://your-app.onrender.com")
        sys.exit(1)
    ...
    try:
        # Wait for Device 3 fault sequence (~370s + buffer)
        threads[2].join(timeout=450)
